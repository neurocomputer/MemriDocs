...
if TEST_READ_WRITE: # read/write tests
    ...
    WL = 0
    BL = 0
    print(f'\nWeight {BL}, {WL} = {device.read_one_weight(BL, WL)[0]}')
...
if TEST_ANN: # two-layer network assembled manually
    print('\n'+50*'*')
    print('manually assembled ANN test')
    print(50*'*')
...
if TEST_ANN_TF: # load network from file
    print('\n'+50*'*')
    print('ANN test with automatic conversion')
    print(50*'*')

    SOURCE_MODEL_PATH = os.path.join('XOR.keras')
